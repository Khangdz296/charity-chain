...
Đồ án đề xuất mô hình sử dụng smart contract để quản lý quỹ từ thiện theo từng cột mốc giải ngân. Khi chiến dịch được tạo, mục tiêu quỹ, danh sách milestone, số tiền, mục đích chi tiêu và danh sách verifier được công bố cố định. Tổ chức từ thiện chỉ có thể nhận tiền theo từng milestone sau khi gửi bằng chứng và trải qua giai đoạn giám sát. Nếu không có phản đối trong thời gian quy định, milestone được release theo cơ chế optimistic release và charity phải gọi `claimMilestone()` để nhận ETH. Nếu có phản đối, milestone chuyển sang trạng thái tranh chấp và cần 2/3 verifier vote resolve trước khi release và claim.
...
4. Nếu không có phản đối, milestone được release sau challenge period.
5. Nếu có phản đối, milestone chuyển sang tranh chấp.
6. Cần 2/3 verifier vote resolve để milestone được tiếp tục release.
7. Sau khi release, charity gọi `claimMilestone()` để nhận ETH.
...
- `Released`: milestone đã được mở khóa để charity claim tiền.
...
- `release()`: chuyển milestone sang trạng thái claimable khi đủ điều kiện.
- `claimMilestone()`: charity nhận ETH của milestone đã release.
- `getMilestone()`: đọc thông tin milestone.

### 5.4. Kiểm soát bảo mật

Hàm `release()` chỉ cập nhật trạng thái milestone thành `Released` và đánh dấu milestone có thể claim. ETH chỉ được chuyển khi charity gọi `claimMilestone()`. Contract dùng khóa `nonReentrant` đơn giản, đồng thời kiểm soát quyền bằng `onlyCharity` và `onlyVerifier`.
...
6. Charity gọi `claimMilestone(0)` để nhận ETH của milestone 0.
...
6. Gọi `release(1)` để mở khóa milestone.
7. Charity gọi `claimMilestone(1)` để nhận ETH.
...
- Bản demo đã có funding deadline và refund cơ bản, nhưng chưa có thay verifier, danh tiếng verifier và quản lý dữ liệu cá nhân đầy đủ.
...
Đồ án đã đề xuất mô hình ứng dụng blockchain trong quản lý dòng tiền từ thiện theo hướng minh bạch hóa quy trình giải ngân. Smart contract giúp cố định kế hoạch chi tiêu, giữ tiền đóng góp, ghi nhận bằng chứng và chỉ cho charity claim tiền khi milestone đạt điều kiện.
...
- hoàn thiện quản trị verifier và cơ chế thay verifier;
